...
import type { ColumnDef, RowSelectionState, SortingState, Table as TanTable } from '@tanstack/react-table';
import {
  createColumnHelper,
  flexRender,
  getCoreRowModel,
...
const columns: ColumnDef<RowData, any>[] = [
  ...
  columnHelper.accessor('title', {
    cell: (info) => {
      ...
      return (
        <MuiLink to={String(scheduleId)} component={RemixLink} onClick={(ev) => ev.stopPropagation()}>
          {info.getValue()}
        </MuiLink>
      );
    },
...
function EnhancedTableToolbar({ selected, table }: { selected: string[]; table: TanTable<RowData> }) {
  ...
  const submit = (action: 'delete' | 'run') => {
    ...
    const selected = table.getSelectedRowModel().rows.map(({ original: { id } }) => id);
    ...
    >
      ...
      {isSelected ? (
        <Box display="flex" gap={2} whiteSpace="nowrap">
          ...
          <Box borderRight={(theme) => `thin solid ${theme.palette.divider}`} height={32} />
          <Button
            variant="text"
            color="inherit"
            onClick={() => {
...
export default function SchedulesTable({ schedules }: { schedules: RowData[] }) {
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar selected={Object.keys(rowSelection)} table={table} />
        <TableContainer>
