...
void (async () => {
  const defaultRegisterJob = path.join('/enschedule/jobs', 'index.js');
  if ((await fs.promises.stat(defaultRegisterJob)).isFile()) {
  ...
  if (process.env.ENSCHEDULE_API) {
    worker.serve({ port: process.env.API_PORT ? Number(process.env.API_PORT) : 8080, hostname: process.env.API_HOSTNAME });
  }
})();
