...
export function RunDetails({ run }: { run: PublicJobRun }) {
  ...
  React.useEffect(() => {
    ...
    if (!run.capabilities.viewLogs) return () => { active = false; };
  ...
  return <Box id="RunPage" display="flex" flexDirection="column" gap={3}>
    ...
    {run.capabilities.viewLogs && <Card><CardContent><Typography variant="h5" gutterBottom>stdout / stderr</Typography>{logs ? <CodeBlock language="text" value={logs} /> : <Typography color="text.secondary">No output.</Typography>}</CardContent><CardActions><Button disabled={!logs} onClick={() => navigator.clipboard.writeText(logs)}>Copy</Button></CardActions></Card>}
  </Box>;
}
